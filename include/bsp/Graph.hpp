...
/*
Class clause describes functional nodes into factor graphs. 
Each object describe a logical operator and stores all information that the algorithm needs.
*/
...
public:
  ...
  unsigned int *_ptr_c() { /*pointer to variable _c*/ return &_c; };
  ...
  friend ostream &operator<<(ostream &out,
                             Clause &C) { /*print clause on file in CNF form.*/
    list<bool>::iterator b = C._lit.begin();
    for (list<Vertex *>::iterator ___it = C.V.begin(); ___it != C.V.end();
         ++___it) {
  ...
  /*remember that vectors only change the values but not the positions, whereas lists can also change the positions*/
  ...
  vector<double *>
      survey_cl_to_i_f_ptr; /*vector of pointers to vector in Vertex*/
  ...
  bool _I_am_in_list_unsat;    /*variables that verifies that a clause is not
                                  satisfied*/
  bool I_am_a_cl_true; /*variable which identifies if the clause is satisfied*/
  bool I_am_white; /*boolean variable thatt identifies if a clause has became
                      "white" or not*/
...
inline int Clause::_cast_b_to_i(
    bool flag) { /*cast a Boolean variable to an integer value*/
  if (flag) {
    return 1; /*true*/
  } else {
    return 0; /*false*/
...
inline bool Clause::_check() {
  ...
  for (unsigned int i = 0; i < cpV.size();
       ++i) { /*loop for checking if a clause is satisfied or not*/
    if (cpV[i]->_I_am_a_fixed_variable) /*all the variables shall be fixed when
                                           the algorithm stops*/
      flag = flag or
             (!(cp_lit[i] xor cpV[i]->_who_I_am)); /*logical operator or between
                                                      literals into a clause*/
  }
  if (flag)
...
/*public member which describe the logical operator into a clause, defining,
 * therefore, the problem*/
inline bool Clause::_logic_operator() { /*logical operator in each clause*/
  bool flag = false;                    /*boolean varoiable*/

#ifdef SAT /*if SAT is defined than SAT problems can be solved using this      \
              code*/
...
// xor sat must be constructed with a different program, I think the equations change

#ifdef XORSAT /*if XORSAT is defined than XORSAT problems can be solved using  \
                 this code*/
  if (V.empty()) { /*this statement it true only when a clause is completely
                      empty. When the condition is true, we can check if the
                      clause is satisfied or not*/
    for (unsigned int i = 0; i < _vb.size();
         ++i) { /*loop for checking if a clause is satisfied or not*/
      flag = flag xor
             _vb[i]; /*logical operator xor between literals into a clause*/
    }
  }
#endif

  /*per il naesat devo invece usare l'or e poi quando una e' singola e
   * soddisfatta devo metterla a 0*/
#ifdef NAESAT /*if NAESAT is defined than NAESAT problems can be solved using  \
                 this code*/
  if (V.size() <=
      _vb.size() - 2) { /*this statement becomes true when at least two literas
                           have been fixed in the same clause*/
    bool _flag_t = false; /*Boolean variable for checking if a true variable has
                             been fixed*/
    bool _flag_f = false; /*Boolean variable for checking if a false variable
                             has been fixed*/
    for (unsigned int i = 0; i < _vb.size();
         ++i) { /*loop for checking if the a clause is satisfied or not*/
      if (_v_ref[i] == 0)
        _flag_f = true; /*checking if at least two literals in the same clause
                           are different*/
      if (_v_ref[i] == 1)
        _flag_t = true; /*checking if at least two literals in the same clause
                           are different*/
      flag =
          flag or _vb[i]; /*logical operator or between literals into a clause*/
    }
    if (!_flag_f or !_flag_t)
      flag = false; /*checking if NAE condition is satisfied*/
  }
#endif
...
public:
  ...
  Graph(int argc, char *const argv[]) {
    ...
    for (int i = 0; i < _argc; ++i)
      _argv[i] = static_cast<string>(argv[i]);
    ...
    _K = 0;   /*number of literals in a cluase set to 0. This is used for K-SAT
                 problems*/
  ...
  void
  update_clause_messages(unsigned int &c); /*update messages for SP algorithm*/

  void unit_propagation(unsigned int &c); /*fix the variable into clause c to
                                             true and clean the graph*/

  void unit_propagation(); /*fix the variable into clause c to true and clean
                              the graph*/
  ...
  void diag_move(const char *action, Vertex *v,
                 int dir); /*log one fix/release*/
  ...
  void print_on_file_residual_formula(); /*print on file residual formula in CNF
                                            form*/
  ...
  void decimate_one(Vertex *v,
                    int force_dir = -1); /*fix one variable (-1: SP rule)*/

  int oracle_dir(Vertex *v); /*oracle-resolved direction, -1 = use SP rule*/

  void
  clean(Vertex *&V_to_clean); /*clean the graph lists of unsitisfied clause*/

  void
  build(Vertex *&V_to_build); /*build the graph lists of unsitisfied clause*/
  ...
  friend ostream &operator<<(ostream &out,
                             Graph &B) { /*operator << for object graph*/
    out << B._N_t << " " << B._M_t << " " << B.complexity << " "
        << B.complexity / (double)B._N << " "
        << (double)(B._M_t) / (double)B._N_t << endl;
    return out;
  }
  ...
  list<Vertex *> _list_fixed_element; /*list of Vertex pointers, i.e fixed
  ...
  bool fl_bsp; /*boolean variable for choosing between becktracking or
                  decimation move*/
...
private:
  ...
  double _pU_j_to_a, _pS_j_to_a,
      _pI_j_to_a;          /*surveys for each variable into the clause*/
  ...
  unsigned int _N_t,
      _M_t; /*number of variable and clauses after t decimations*/
  ...
  bool _in_trial; /*true inside a forked probe child: suppress diag writes*/
  ...
  void WellRandomInitialization() { /*initialization random number generator*/
    if (g_fixed_seed >= 0)
      _seed = static_cast<unsigned>(g_fixed_seed); /*reproducible runs*/
    else
      _seed = GetRandom(); /*call and store seed*/
    if (_seed < 1)
      exit(-1);                 /*check if seed is ok*/
  ...
  void _update_message(unsigned int &c, list<double *>::iterator &s_i,
                       list<Vertex *>::iterator &it_cl);
  ...
  struct _Vertex_greater_pred { /*predicate for sort on a vector of Vertex*/
    bool operator()(Vertex *x, Vertex *y) { return x->_sC > y->_sC; }
  };

  struct _Vertex_smaller_labeled_vertex_pred { /*predicate for sort on a vector
                                                  of Vertex*/
    bool operator()(Vertex *x, Vertex *y) { return x->_vertex < y->_vertex; }
  };

  struct _Vertex_is_fixed_pred { /*predicate: fixed variables first*/
    bool operator()(Vertex *x) { return x->_I_am_a_fixed_variable; }
  };
};

inline double Graph::_Pr_U(Vertex *V,
                           bool b) { /* product of unsitisfied messages*/
  return (b) ? (V->prod_V_minus) : (V->prod_V_plus);
}

/*public member which returns, depending by the literal into a clause, the right
 * value of products of (1-message) for SP and BP equations*/
inline double Graph::_Pr_S(Vertex *V, bool b,
                           double &s) { /*product of sitisfied messages*/
...
inline double Graph::Div_s(double &s) { return 1. / (1. - s); }
...
inline double Graph::_p_U(double &PU,
                          double &PS) { /*compute survey sF clause to variable*/
  return ((1. - PU) * PS);
}

inline double Graph::_p_S(double &PU,
                          double &PS) { /*compute survey sT clause to variable*/
  return ((1. - PS) * PU);
}

inline double Graph::_p_I(double &PU,
                          double &PS) { /*compute survey sI clause to variable*/
  return (PU * PS);
}
// e' sbagliata!!!!
inline void Graph::_update_message(unsigned int &c,
                                   list<double *>::iterator &s_i,
                                   list<Vertex *>::iterator &it_cl) {
  exit(-1);
  double a = **s_i;
  list<double *>::iterator
      s_j; /*complex pointer iterator for literals j into a clause*/
  ...
  s_j =
      _cl[c]
          .survey_cl_to_i.begin(); /*iterator to first message into the clause*/
  for (list<Vertex *>::iterator it_cl_j = _cl[c].V.begin();
       it_cl_j != _cl[c].V.end(); ++it_cl_j) {
    if ((*it_cl)->_vertex != (*it_cl_j)->_vertex) {
      // _new*=compute_message(it_cl_j,s_j, _lit_it_j);/*new message from cl to
  ...
  _new =
      (_new < ZERO)
          ? (0.)
          : (_new); /*set to 0 a message iff the message is smaller than 1e-16*/
...
// devo capire bene come affrontare questo punto. Devo usare le equazioni
// scrittee su mezared et al.

inline double Graph::compute_message(double &s, double &u) {
  _new = (s / u);
  return (_new < ZERO) ? (0.)
                       : (_new); /*new message from cl to variable is computed*/
}

inline double Graph::__pu() { return (1. - _prod_U) * _prod_S; }
