...
Una politica $\pi(\cdot\mid F)$ è una distribuzione sulle azioni.
SAC massimizza la somma delle ricompense più l'entropia della politica, pesata con una temperatura $\alpha>0$.
...
\end{equation}
La politica di SAC è quindi la distribuzione di Gibbs dei valori $Q_\alpha$, e vale l'identità $Q_\alpha-V_\alpha=\alpha\log\pi_\alpha$.
...
Per $\alpha\to0$ esso tende a $\max_aQ_a$, e la politica diventa la scelta deterministica della migliore azione.
...
Ng, Harada e Russell~\citep{ng1999policy} mostrano che una ricompensa di questo tipo non cambia la politica ottima.
